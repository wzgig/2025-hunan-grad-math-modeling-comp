...
cfg.n_devices_OAT  = 100;   % OAT用更少设备数；60或40都行
...
k = 1;
add('tau_A', 'min', params.tt.A, 0.15, @(P,sgn) setfield_tt(P,'A',P.tt.A*(1+sgn*0.10)));
add('tau_B', 'min', params.tt.B, 0.15, @(P,sgn) setfield_tt(P,'B',P.tt.B*(1+sgn*0.10)));
add('tau_C', 'min', params.tt.C, 0.15, @(P,sgn) setfield_tt(P,'C',P.tt.C*(1+sgn*0.10)));
add('tau_E', 'min', params.tt.E, 0.15, @(P,sgn) setfield_tt(P,'E',P.tt.E*(1+sgn*0.10)));

add('setup_A', 'min', params.setup.A, 0.25, @(P,sgn) setfield_setup(P,'A',max(1,round(P.setup.A*(1+sgn*0.20)))));
add('setup_B', 'min', params.setup.B, 0.25, @(P,sgn) setfield_setup(P,'B',max(1,round(P.setup.B*(1+sgn*0.20)))));
add('setup_C', 'min', params.setup.C, 0.25, @(P,sgn) setfield_setup(P,'C',max(1,round(P.setup.C*(1+sgn*0.20)))));
add('setup_E', 'min', params.setup.E, 0.25, @(P,sgn) setfield_setup(P,'E',max(1,round(P.setup.E*(1+sgn*0.20)))));

add('r2_E', 'prob', params.fail_r2.E, 0.25, @(P,sgn) setfield_r2(P,'E',clip01(P.fail_r2.E*(1+sgn*0.20))));
add('r1_E', 'prob', params.fail_r1.E, 0.25, @(P,sgn) setfield_r1(P,'E',clip01(P.fail_r1.E*(1+sgn*0.20))));
